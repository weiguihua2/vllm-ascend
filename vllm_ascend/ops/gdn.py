...
def _check_and_get_host_args(attn_metadata, field_name: str, sub_field_name: str):
    if (host_meta := getattr(attn_metadata, field_name, None)) is None:
        raise RuntimeError(f"Expected attn_metadata.{field_name}.{sub_field_name} for Ascend GDN host metadata path.")
    return host_meta


def get_non_spec_causal_conv1d_host_args(attn_metadata) -> tuple[tuple[int, ...], tuple[int, ...], tuple[int, ...]]:
    host_meta = _check_and_get_host_args(attn_metadata, "non_spec_prefill_host_meta", "causal_conv1d")
...
class AscendGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def _forward_core(
        ...
        if attn_metadata.num_prefills > 0:
            ...
            # Derive this inside the captured graph. The metadata object is
            # rebuilt per batch, while FULL_DECODE_ONLY replays this branch.
