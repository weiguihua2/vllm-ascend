...
class AscendGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def _forward_core(
        ...
        # if tp_rank == 0 and pcp_rank == 1:
        #     logger.info(f"======= gdn, num_actual_tokens: {num_actual_tokens}, mixed_qkv_sum: {mixed_qkv.flatten(1).sum(dim=1)}, mixed_qkv: {mixed_qkv}")
        ...
        # if tp_rank == 0 and pcp_rank == 1:
        #     logger.info(f"====== conv, sum: {mixed_qkv_non_spec.flatten(1).sum(dim=1)}, mixed_qkv_non_spec: {mixed_qkv_non_spec}")
        ...
        if attn_metadata.num_prefills > 0:
            ...
            # if tp_rank == 0 and pcp_rank == 1:
            #     logger.info(f"======= after npu_recurrent_gated_delta_rule (decode), "
            #                  f"shape={core_attn_out_non_spec.shape}, "
            #                  f"sum={core_attn_out_non_spec.flatten(1).sum(dim=1)}, "
            #                  f"has_nan={bool(torch.isnan(core_attn_out_non_spec).any())}, "
            #                  f"has_zero={bool((core_attn_out_non_spec == 0).all())}, "
            #                  f"core_attn_out_non_spec: {core_attn_out_non_spec}")
        else:
        ...
        # if tp_rank == 0 and pcp_rank == 1:
        #     logger.info(f"========= gateDelta, num_actual_tokens: {num_actual_tokens}, "
        #                  f"sum: {core_attn_out[:num_actual_tokens].flatten(1).sum(dim=1)}, "
        #                  f"has_nan={bool(torch.isnan(core_attn_out).any())}, "
        #                  f"has_zero={bool((core_attn_out[:num_actual_tokens] == 0).all())}")
